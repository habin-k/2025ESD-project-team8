...
# --------- Letterbox 함수 ---------
def letterbox_image(image, size=(512, 512)):
    h, w = image.shape[:2]
...
core = Core()
model_ov = core.read_model("yolo11n-pose_openvino_512/yolo11n-pose.xml")
...
try:
    while True:
        ...
        # 512 x 384로 resize
        frame = cv2.resize(frame, (512, 0.75 * 512), interpolation=cv2.INTER_LINEAR)

        # Letterbox 처리 (512 x 512로 입력 맞춤)
        input_img, scale, pad_x, pad_y = letterbox_image(frame, size=(512, 512))
